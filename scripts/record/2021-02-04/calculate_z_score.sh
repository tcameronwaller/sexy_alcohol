#!/bin/bash

# Organize variables.
column=$1 # column for which to calculate z-score
path_table_original=$2 # original table
path_table_novel=$3 # novel table

# Calculate cumulative statistics.
count=$(cat $path_table_original | awk 'NR > 1 { n++ } END { print n }')
sum=$(cat $path_table_original | awk -v column=$column 'NR > 1 { sum += $column } END { print sum }')
sum_squares=$(cat $path_table_original | awk -v column=$column 'NR > 1 { sum_squares += ($column)^2 } END { print sum_squares }')
mean=$(cat $path_table_original | awk -v column=$column 'NR > 1 { sum += $column; n++ } END { if (n > 0) print (sum / n); }')
variations=$(cat $path_table_original | awk -v column="$column" -v mean="$mean" \
'NR > 1 { value += (($column - mean)^2) } END { print value }')
# The following formulas for standard deviation are equivalent.
#standard_deviation=$(bc -l <<< "sqrt(($sum_squares - (($sum^2)/$count))/($count - 1))")
standard_deviation=$(bc -l <<< "(sqrt(($variations)/($count - 1)))")

# Report statistics.
#echo "Count: ${count}"
#echo "Sum: ${sum}"
#echo "Sum of squares: ${sum_squares}"
#echo "Mean: ${mean}"
#echo "Standard deviation: ${standard_deviation}"

#head $path_table_original
rm $path_table_novel
cat $path_table_original | awk 'NR == 1' > $path_table_novel
cat $path_table_original | \
awk -v column="$column" -v mean="$mean" -v stdev="$standard_deviation" \
'NR > 1 {print $1, $2, $3, $4, (($column - mean) / stdev), $6}' >> $path_table_novel
#head $path_table_novel
